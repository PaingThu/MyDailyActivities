import{t as e}from"./modulepreload-polyfill--sBP-D0G.js";import{a as t,i as n,n as r,o as i,r as a,t as o}from"./config-Cdkeo--o.js";var s=e((async()=>{await n();var e=document.getElementById(`authenticating`),s=document.getElementById(`mainContainer`),c=document.getElementById(`userForm`),l=document.getElementById(`submitBtn`),u=document.getElementById(`btnText`),d=document.getElementById(`btnLoader`),f=document.getElementById(`statusMessage`),p=document.getElementById(`displayArea`),m=document.getElementById(`user`),h=document.getElementById(`type`),g=document.getElementById(`distanceContainer`),_=document.getElementById(`distance`),v=document.getElementById(`loadDataBtn`),y=document.getElementById(`logoutBtn`),b=document.getElementById(`adminName`),x=document.getElementById(`timeInputContainer`),S=document.getElementById(`happyBirthday`),C=document.getElementById(`userPage`);C.onclick=()=>{window.open(a(),`_blank`)};var w=new Date().toLocaleDateString(`ja-JP`,{year:`numeric`,month:`2-digit`,day:`2-digit`}).split(`/`).join(`-`);document.getElementById(`date`).value=w;var T=[`00:05`,`00:10`,`00:15`,`00:30`,`00:45`,`01:00`,`01:15`,`01:30`,`01:45`,`02:00`,`02:15`,`02:30`,`02:45`,`03:00`];async function E(){if(!`https://script.google.com/macros/s/AKfycbzgsuA7_mjRWvpcuagCPKYlE91vxq0V2M4XGXT9udM3yLZ2mvr8TBL0eFYv7VvoVnpK/exec`.includes(`YOUR_WEB_APP_URL`)){p.innerHTML=t(`Data Loading .....`);try{k((await(await fetch(r)).json()).sort((e,t)=>{let n=e[3],r=t[3];return n.localeCompare(r)}))}catch(e){console.error(`Fetch error:`,e),p.innerHTML=`<p class="text-red-500 text-center py-10 text-sm">Error connecting to database.</p>`}}}v.addEventListener(`click`,E);var D=async()=>{let t=await i(`admin`);t&&t.status===`success`&&(b.textContent=t.name,m.value=t.user,t.birthdayInfo.today&&M(t.birthdayInfo),e.classList.add(`hidden`),s.classList.remove(`hidden`),E())};document.readyState===`loading`?document.addEventListener(`DOMContentLoaded`,D):D();function O(){let e=h.value;[`Walking`,`Running`,`Cycling`].includes(e)?(g.classList.remove(`hidden`),_.required=!0,x.innerHTML=`
